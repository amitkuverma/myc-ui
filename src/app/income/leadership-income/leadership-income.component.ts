...
@Component({
  selector: 'app-leadership-income',
  standalone: true,
  imports: [CommonModule, FormsModule, NgxPaginationModule],
  templateUrl: './leadership-income.component.html',
  styleUrl: './leadership-income.component.scss'
})
export class LeadershipIncomeComponent {
  ...
  fetchUsers(): void {
    ...
    this.transactionService.getAllTransaction().subscribe((data: any) => {
      if (this.cookies.isAdmin()) {
        const adminHistory = data.filter((item:any) => item.paymentType === 'leadership' );
        this.transInfo = adminHistory;
        this.filteredTrans = adminHistory;
        this.totalItems = adminHistory.length;

      } else {
        const userHistory = data.filter((item:any) => item.userId === this.cookies.decodeToken().userId && item.paymentType === 'leadership' );
